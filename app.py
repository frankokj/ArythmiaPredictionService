...
def uploaddata():
    if request.method == 'POST':
        ...
        strData = req_data['result']
        # Save the file to ./uploads


        pred = model_predict_data(strData, model)
...
def model_predict_data(strData, model):
    flag = 1
    ...
    for t in [1]:
        ...
        csv = strData
        #csv_data = csv.split(',')
        data = np.fromstring(csv, dtype=float, sep=',')
       # data = np.array(list)
        signals = []
        count = 1
        peaks = biosppy.signals.ecg.christov_segmenter(signal=data, sampling_rate=200)[0]
        ...
        for count, i in enumerate(signals):
            ...
            pred_class = pred.argmax(axis=-1)
            if pred_class == 0:
                apcv = str(indices[count])
                apcj = {"val": apcv}
                APC.append(apcj)
            elif pred_class == 1:
                #NORMAL.append(indices[count])
                norv = str(indices[count])
                norj = {"val": norv}
                NORMAL.append(norj)
            elif pred_class == 2:
                #LBB.append(indices[count])
                lbbv = str(indices[count])
                lbbj = {"val": lbbv}
                LBB.append(lbbj)
            elif pred_class == 3:
                #PAB.append(indices[count])
                pabv = str(indices[count])
                pabj = {"val": pabv}
                PAB.append(pabj)
            elif pred_class == 4:
                #PVC.append(indices[count])
                pvcv = str(indices[count])
                pvcj = {"val": pvcv}
                PVC.append(pvcj)
            elif pred_class == 5:
               # RBB.append(indices[count])
               rbbv = str(indices[count])
               rbbj = {"val": rbbv}
               RBB.append(rbbj)
            elif pred_class == 6:
               # VEB.append(indices[count])
                vebv = str(indices[count])
                vebj = {"val": vebv}
                VEB.append(vebj)
